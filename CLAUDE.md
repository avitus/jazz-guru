...
- **`actions/` — tool registry + controller.**
  ...
  - `sandbox.resolve_in_workspace` and `sandbox.resolve_in_safe` are the two sanctioned ways to convert a user/tool-supplied path into a real one. `resolve_in_workspace` is strict — refuses anything outside `<workspace>/sessions/<sid>/` — and is required for **writes**. `resolve_in_safe` additionally permits reads under `data/`, `JG_INSTRUMENTS_ROOT`, and `JG_SAFE_EXTRA_PATHS`; use it for read-only tools (`fs_read`, `fs_list`, `lick_match`, `presets`). See "Conventions for new code" below for the full rules.
...
- **`lickmatch.py`** + **`actions/tools/lick_match.py`** — typed surface over the Weimar Jazz Database (`data/wjazzd/wjazzd-index.json` + `wjazzd.db`, ODbL). `lick_match(midi_path | notes | intervals+iois)` + `lick_match_info()` are the recommended path for **melodic similarity** queries (they compute n-gram interval/IOI features the raw JSON doesn't expose). The corpus itself is core to the agent's musical knowledge, so the raw files at `data/wjazzd/` are also reachable directly via `fs_read` / `fs_list` / `python_exec` (a safe root — see the sandbox notes below). Every `lick_match` result carries WJazzD attribution (performer + title + year) per the ODbL; the agent should propagate attribution when surfacing matches. Wired into the `music` toolset.
...
- **Tools**: define a Pydantic input model for `input_model=`, register with snake_case names, and pull the session id from `actions.context.current()` rather than accepting it as an arg. Sandbox helpers in `actions/sandbox.py`:
  ...
  - **`resolve_in_safe(path, sid)`** — read-oriented; additionally allows `data/`, `JG_INSTRUMENTS_ROOT`, and any `JG_SAFE_EXTRA_PATHS`. Relative paths try the session workspace first, then the project root (so the agent can say `data/wjazzd/wjazzd-index.json` without an absolute path); whichever anchor lands under a safe root wins. Absolute paths must fall under a safe root.
  - `fs_read` and `fs_list` use `resolve_in_safe`, so the agent can browse any safe root directly — including the WJazzD corpus at `data/wjazzd/`, which is core to its musical knowledge. Writes (`fs_write`) stay strict.
  - Tools whose data benefits from a typed surface (presets, melodic similarity via `lick_match`) still get one — typed tools are better than raw `fs_read` when the operation is structured. Use the dedicated surface when one exists; reach for `fs_read` for free-form exploration.
